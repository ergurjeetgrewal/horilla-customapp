...
def login_user(request):
    ...
    return render(
        request, "mlogin.html", {"initialize_database": initialize_database_condition()}
    )


def OauthLogin(request):
    """
    Servers page for Google Oauth Login.
    """
    if request.method == "POST":
        ...
        google_auth_url = getattr(settings, "GOOGLE_AUTH_URI", "")
        params = {
            "client_id": client_id,
...
# To get new badge id for employee profile
def badge_id_generator():
    last_employee = Employee.objects.all().order_by("id").last()
    if last_employee and last_employee.badge_id:
        prefix = "".join(filter(str.isalpha, last_employee.badge_id))  # Extracts 'PEP'
        numeric_part = "".join(
            filter(str.isdigit, last_employee.badge_id)
        )  # Extracts '0005'
        new_numeric_part = int(numeric_part) + 1
        # Format the new badge_id
        new_badge_id = f"{prefix}{new_numeric_part:04d}"  # Zero-padded to 4 digits
    else:
        # If there are no employees or no badge_id, start with the first badge_id
        new_badge_id = "PEP0001"

    return new_badge_id


def EmployeeOAuthLogin(request):
    """
    ...
    try:
        # to match the token time with google oauth and system
        time.sleep(0.5)
        idinfo = id_token.verify_oauth2_token(
            id_token_value, google_requests.Request(), client_id
        )
        if idinfo.get("hd") != "thecodeinsight.com":
            messages.warning(
                request, _("Access Denied: Only Employees permitted to login.")
            )
            return redirect("/login")
        ...
        if created:
            """
            Tables created in order for new user as a normal employee
            """
            change_ownprofile_permission = Permission.objects.get(
                codename="change_ownprofile"
            )
            view_ownprofile_permission = Permission.objects.get(
                codename="view_ownprofile"
            )
            user.user_permissions.add(
                change_ownprofile_permission, view_ownprofile_permission
            )
            user.save()
            new_badge_id = badge_id_generator()
            ...
            """
            Threadprocess handling EmployeeWorkinformation profile creation 
            Updating Company id to work profile manually
            """
            company = Company.objects.filter(id=1).first()
            workinfo = EmployeeWorkInformation.objects.filter(
        ...
        print(e)
        messages.error(request, _("Access Denied. Please Connect with administrator"))
        return redirect("/login/")
